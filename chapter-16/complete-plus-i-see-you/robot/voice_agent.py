...
class VoiceAgent:
    def __init__(self):
        ...
        self.waiting_to_see_face = False

    def on_connect(self, client, userdata, flags, rc):
        print("Connected to MQTT broker with result code", rc)
        client.subscribe("face_detector/faces")
        client.message_callback_add("face_detector/faces", self.handle_faces)

    def handle_faces(self, client, userdata, message):
        if self.waiting_to_see_face:
            payload = json.loads(message.payload)
            if payload and len(payload) > 0:
                self.synthesizer.speak("I see you !")
                self.waiting_to_see_face = False
